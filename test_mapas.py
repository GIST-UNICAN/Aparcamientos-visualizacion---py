...
#    chrome_options.binary_location = CHROME_PATH
    ventana_mapas = Toplevel()
    ventana_mapas.withdraw()
    ventana_carga= Toplevel()
    ventana_carga.update()
    ventana_mapas.title("Mapas vehiculo " + str(dataframe.index))
...
#    driver.set_window_size(height=680, width=530)  # choose a resolution
    ...
    for elemento,iteracion in zip(utilidades, count(0)):
        ...
        driver.get(r"E:\OneDrive - Universidad de Cantabria\Recordar GIST - VARIOS\Aparcamientos\SCRIPTS PARK\mapa{}.html".format(str(iteracion)))
        driver.save_screenshot('screenshot{}.png'.format(str(iteracion)))
    driver.close()
    frame = HtmlFrame(ventana_mapas, horizontal_scrollbar="auto")
